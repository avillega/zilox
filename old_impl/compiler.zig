...
const Compiler = struct {
    ...
    pub fn verifyRedeclartion(self: *Compiler, name: Token) CompileError!void {
        ...
        while (i >= 0) : (i -= 1) {
            ...
            if (std.mem.eql(u8, local.name.lexeme, name.lexeme)) {
                return CompileError.AttemptVariableRedeclare;
            }
        }
    }

    pub fn addLocal(self: *Compiler, name: Token) CompileError!void {
        if (self.local_count == u8_count) {
            return CompileError.TooManyLocals;
        }
        try self.verifyRedeclartion(name);
...
const Parser = struct {
    ...
    fn block(self: *Self) !void {
        while (!self.check(.RIGHT_BRACE) and self.scanner.hasNextToken()) {
    ...
    fn declareVariable(self: *Self) !void {
        ...
        current.addLocal(name) catch |err| {
            const error_msg = switch (err) {
                .TooManyLocals => "Too many local variables in function.",
                .AttemptVariableRedeclare => "Already a variable with this name in this scope.",
                else => unreachable,
            };
